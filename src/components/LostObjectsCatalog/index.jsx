...
export default function LostObjectCatalog() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const objectsResponse = await axios.get(`https://10.182.0.39/api/lost-objects/user/${token}`);
  ...
  const handleCreateSubmit = async (event) => {
    ...
    try {
        const response = await axios.put(`https://10.182.0.39/api/lost-objects/${getLostObjectID(objectName,objects)}`);
    } catch (error) {
        console.error(error);
