...
export default function AddCategoryComponent() {
  ...
  if (categories.length === 0){
   ...
   // Função para buscar subcategorias de uma categoria
    const fetchSubCategories = async (data) => {
      try {
        const response = await axios.get(`https://bidfinderbackend.ddns.net/api/category/subCat/${data}`);
  ...
  const handleCreateSubSubCategory = async (event) => {
    ...
    try {
        const response = await axios.post(`https://bidfinderbackend.ddns.net/api/category/subSubCat/${getCategoryNameFromId(mainCategory)}/${getSubCategoryNameFromId(subMainCategory)}`,{"name": newSubSubCategoryToCreate});
...
const handleDeleteSubSubCategory = async (event) => {
  ...
  try {
      const response = await axios.delete(`https://bidfinderbackend.ddns.net/api/category/subSubCat/${getCategoryNameFromId(mainCategoryToDelete)}/${getSubCategoryNameFromId(subMainCategoryToDelete)}/${getSubSubCategoryNameFromId(subSubMainCategoryToDelete)}`);
    console.log(response)
  } catch (error) {
      console.error( error);
