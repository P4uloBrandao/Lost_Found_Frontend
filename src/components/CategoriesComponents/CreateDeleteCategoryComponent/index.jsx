...
export default function AddCategoryComponent() {
  ...
  const [category, setCategory] = useState('');
    ...
    const handleCreateSubmit = async (event) => {
        ...
        try {
            const response = await axios.post(`https://10.182.0.39/api/category/`,{"name": categoryToCreate});
    ...
    const handleDeleteSubmit = async (event) => {
        event.preventDefault();
        try {
            const response = await axios.delete(`https://10.182.0.39/api/category/${categoryToDelete}`,);
