  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const userResponse = await axios.get(`https://10.182.0.39/api/users/profile/${token}`);
        const userData = userResponse.data.currentUser._id;
    
        // Buscar os dados dos leilões
        const auctionsResponse = await axios.get(`https://10.182.0.39/api/auction/user/${userData}`);
        ...
        auctionsData.forEach(auction => {
          ...
          const requestPromise = axios.get(`https://10.182.0.39/api/found-objects/${foundObject}`);
          requests.push(requestPromise);
          const requestPromiseBid = axios.get(`https://10.182.0.39/api/auction/${auction._id}/bid`);
